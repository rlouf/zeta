...
from commas.display.tty import iris_italic, is_interactive, muted, should_color
...
def progress_mode_from_env(env: Mapping[str, str] | None = None) -> str:
    ...
    mode = values.get("COMMAS_PROGRESS", PROGRESS_MODE_COMPACT).lower()
...
def create_stream_renderer(output: TextIO) -> StreamRenderer:
    """Return the renderer Commas should use for human assistant text."""
...
class ThinkingStatus:
    ...
    recorded in the trace and rendered by `commas session transcript`.
    """
...
def thinking_trace_enabled(env: Mapping[str, str] | None = None) -> bool:
    ...
    values = os.environ if env is None else env
    return values.get("COMMAS_THINKING_TRACE", "1").lower() not in {"0", "false"}
