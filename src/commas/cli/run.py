...
from commas.handoff import matching_pending_handoff
...
@cli.command(
    ...
    epilog=examples(
        "commas run -- cargo test",
        "commas run --shell 'pytest -q 2>&1 | tail -20'",
    ),
)
@click.pass_context
@click.option(
...
def cmd_run(
    ...
    Commas flags must come before the command; everything after the first
    ...
    try:
        ...
        stderr = (
            f"commas: missing executable: {program}\n"
        ...
        stderr = f"commas: permission denied: {target}\n"
        click.echo(stderr, err=True, nl=False)
        record_turn(
            command,
            EXIT_COMMAND_NOT_EXECUTABLE,
...
def write_handoff_resume(path: Path | None, command: str, status: int) -> None:
    ...
    if not matching_pending_handoff(
        command,
        current_timeline(runtime_context=zeta_session_for_commas()),
    ):
        return
...
def configured_capture_bytes() -> int:
    raw = os.environ.get("COMMAS_RUN_CAPTURE_BYTES", str(DEFAULT_CAPTURE_BYTES))
    try:
...
def configured_shell_executable() -> str:
    """Return the shell executable used for `commas run --shell`."""
    return os.environ.get("COMMAS_RUN_SHELL") or os.environ.get("SHELL") or ""
