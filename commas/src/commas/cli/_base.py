"""Root Click group and process entrypoint for the Commas CLI.
...
@click.version_option(__version__, "-V", "--version", prog_name="commas")
@click.pass_context
def cli(ctx: click.Context) -> None:
    """Natural-language shell assistant.

    Commas installs punctuation glyphs into your shell, plus named commands for
    ...
    # writes.
    from commas.sessions import ingest_spooled_turns
...
def main(argv: list[str] | None = None) -> int:
    """Parse the shell-agnostic Commas CLI surface."""
    try:
        result = cli.main(args=argv, prog_name="commas", standalone_mode=False)
        ...
        if isinstance(error, RuntimeError):
            click.echo(f"commas: {error}", err=True)
            click.echo(
                "Check the model endpoint with `commas doctor`, then retry.", err=True
            )
            return EXIT_MODEL_UNAVAILABLE
        if isinstance(error, FileNotFoundError):
            program = error.filename or "required executable"
            click.echo(f"commas: missing executable: {program}", err=True)
