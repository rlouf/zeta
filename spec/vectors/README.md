...
Eight prompts generated by the Python runtime with a fixed date and working
...
The cases cover minimal and repaired tool histories, the fifty-event history
boundary, private trace filtering, authorized content ordering, structural
trimming, and drop-oldest compaction.

## `agent/invocations.json`

Ten deterministic agent runs generated by the Python runtime. Scripted model
and tool results cover streaming, tool execution and rejection, native and
Codex tool presentations, effect ordering, duplicate calls, event controls,
cancellation boundaries, and content query, transform, and finish operations.
Each case pins journal drafts, observations, control requests, step order, and
the trace objects and derivations produced by the run.
