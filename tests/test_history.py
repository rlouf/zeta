...
def test_history_query_turns_filters_workflow_outcome_and_since() -> None:
    append_history_record(sample_turn_record("turn-ask", workflow="ask", time=100.0))
    append_history_record(
        sample_turn_record(
            "turn-broken",
            workflow="do",
...
def test_history_query_turns_scopes_by_session_and_touched_path() -> None:
    append_history_record(sample_turn_record("turn-here", time=100.0, session="here"))
    append_history_record(sample_turn_record("turn-there", time=200.0, session="there"))
    append_history_record(
        sample_effect_record(
            "effect-write",
            turn_id="turn-here",
...
def test_history_turn_ids_with_prefix_lists_matches_sorted() -> None:
    append_event(sample_turn_record("aaaa-1111"))
...
def seed_show_and_blame_history(monkeypatch) -> None:
    ...
    append_history_record({**record, "time": 100.0})
    append_history_record(
        sample_effect_record(
            "effect-edit",
            turn_id="turn-do-1111",
...
def seed_bundle_state(monkeypatch) -> dict[str, str]:
    ...
    from zeta.records.stores import SqliteObjectStore, zeta_sqlite_path
    ...
    store = SqliteObjectStore(zeta_sqlite_path(), session_id="bundle-src")
...
def test_bundle_export_honors_since_and_session_filters(monkeypatch) -> None:
    ...
    append_history_record(
        sample_turn_record("turn-late", time=900.0, session="late-session")
    )
