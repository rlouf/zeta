...
that cash that design in: `traces diff` and `traces replay`.

All output below is real, captured against a recorded session. To run it
yourself, point `ZETA_SESSION_ID` at any session with agent turns
(for example, from `zeta traces log --all-sessions`) and have your model endpoint up.

## 1. Find a prompt

`traces log` is the front door; `--kind prompt` narrows it to requests:

```text
$ zeta traces log --kind prompt --limit 3
...
```text
$ zeta traces diff 0c56ad85 2fc6c722 --stat
...
`traces replay` rebuilds the request from the linked components — not
...
```text
$ zeta traces tree 13b60e1a --down --depth 1
