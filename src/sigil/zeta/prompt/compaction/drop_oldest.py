"""Last-resort compaction: drop the oldest transcript messages."""
...
class DropOldestPromptTransform:
    """Drop the oldest transcript messages until the prompt fits the budget."""
    ...
    def apply(self, components: list[PromptComponent]) -> list[PromptComponent]:
        ...
        while measure(output).total_tokens > self.max_tokens:
            remaining = without_oldest_transcript_message(output)
...
def without_oldest_transcript_message(
    components: list[PromptComponent],
) -> list[PromptComponent] | None:
    """Drop the oldest transcript message, taking its tool results along.
    ...
    for index, component in enumerate(components):
        if component.kind != "transcript_message" or component.message is None:
            continue
