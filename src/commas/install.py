"""Install and diagnose Commas shell integrations."""
...
def zsh_binding_source() -> Path:
    ...
    packaged = importlib.resources.files("commas").joinpath("bindings", BINDING_NAME)
    try:
...
def default_zsh_install_dir(env: dict[str, str] | None = None) -> Path:
    ...
    override = values.get("COMMAS_SHELL_DIR")
    if override:
        return Path(override)
    return Path.home() / ".commas" / "shell" / "zsh"
...
def source_snippet(
    ...
    *,
    ...
    commas_bin: str | None = None,
) -> str:
    """Return the rc block that loads a Commas shell binding."""
    reference = shell_reference(binding_path)
    lines = ["", "# Commas", f"if [[ -r {reference} ]]; then"]
    if commas_bin:
        lines.append(f"  export COMMAS_BIN={shlex.quote(commas_bin)}")
    lines.append(f"  export COMMAS_ENABLE_GLYPHS={1 if enable_glyphs else 0}")
    lines.append(f"  source {reference}")
    lines.append("fi")
    return "\n".join(lines) + "\n"


def replace_commas_source_block(
    rc_text: str,
    references: set[str],
    snippet: str,
) -> tuple[str, bool]:
    """Replace an existing Commas rc block that sources this binding."""
    lines = rc_text.splitlines(keepends=True)
    for start, line in enumerate(lines):
        if line.strip() != "# Commas":
            continue
...
def install_zsh_binding(
    ...
    *,
    ...
    snippet = source_snippet(
        ...
        commas_bin=shutil.which("commas"),
    )
...
def check_commas_installed() -> DoctorCheck:
    """Check that the public Commas command is available."""
    check = check_configured_executable("commas", "COMMAS_BIN")
    return DoctorCheck(
        name="commas:installed",
        status=check.status,
        detail=check.detail,
        hint=check.hint,
    )
...
def check_state_writable() -> DoctorCheck:
    """Check that Commas can write its state directory."""
    root = state_dir()
    try:
...
def check_shell_support(shell: str | None) -> DoctorCheck:
    ...
    if shell is None:
        return DoctorCheck(
            ...
            "warn",
            "SHELL is not set",
            "Commas supports zsh; run it in zsh.",
        )
    return DoctorCheck(
        "shell:supported",
        "fail",
...
def check_shell_binding_loaded(env: dict[str, str] | None = None) -> DoctorCheck:
    ...
    values = env if env is not None else os.environ
    binding = values.get("COMMAS_BINDING_LOADED")
    if binding:
        return DoctorCheck("shell:binding-loaded", "ok", f"{binding} binding loaded")
    session_id = values.get("COMMAS_SESSION_ID")
    if session_id:
        return DoctorCheck("shell:binding-loaded", "ok", f"session {session_id}")
    return DoctorCheck(
        "shell:binding-loaded",
        "warn",
        "COMMAS_SESSION_ID is not set",
        "Restart the shell or source the Commas binding.",
    )
...
def check_session_tty(env: dict[str, str] | None = None) -> DoctorCheck:
    ...
    """
    values = env if env is not None else os.environ
    session = values.get("COMMAS_SESSION_ID")
    recorded = values.get("COMMAS_SESSION_TTY")
    ...
    return DoctorCheck(
        ...
        "Re-source the Commas binding or restart the shell.",
    )
...
def check_glyphs_enabled(env: dict[str, str] | None = None) -> DoctorCheck:
    ...
    values = env if env is not None else os.environ
    if not values.get("COMMAS_SESSION_ID"):
        return DoctorCheck(
    ...
    enabled = values.get("COMMAS_ENABLE_GLYPHS", "1").lower() not in {"0", "false"}
    if enabled:
    ...
    return DoctorCheck(
        ...
        "warn",
        "glyphs disabled",
        "Run commas install --glyphs, then restart.",
    )


def doctor_checks() -> list[DoctorCheck]:
    """Run Commas environment checks."""
    checks = [
        check_commas_installed(),
        check_endpoint(),
        *codex_auth_checks(),
