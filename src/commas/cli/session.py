...
@cli.group(
    ...
    epilog=examples(
        "commas session",
        "commas session transcript --limit 20",
        'commas session rename "frontend work"',
        "commas session clear",
    ),
)
...
def cmd_session(ctx: click.Context, json_output: bool) -> None:
    ...
    A session is one terminal: the zsh binding sets COMMAS_SESSION_ID per
...
def session_show(json_output: bool) -> int:
    ...
    each continuity file present. Bare `commas session` runs this command.
    """
...
@cmd_session.command(
    "list",
    epilog=examples("commas session list"),
)
@click.option("--json", "json_output", is_flag=True, help=JSON_HELP)
...
@cmd_session.command(
    "clear",
    epilog=examples("commas session clear"),
)
@click.option("--json", "json_output", is_flag=True, help=JSON_HELP)
...
def session_transcript(limit: int | None, json_output: bool) -> int:
    ...
    `commas trace show`; model reasoning appears in full as italic
...
def print_session_transcript(limit: int | None, json_output: bool) -> int:
    ...
    # Imported lazily: `commas.cli` must not load zeta or rich at import time.
    from commas import zeta_session_for_commas
    from commas.agent_io import current_timeline
    from zeta.records.events import event_view

    events = [
        event_view(event)
        for event in current_timeline(runtime_context=zeta_session_for_commas())
    ]
    ...
    from commas.display.render import render_transcript
