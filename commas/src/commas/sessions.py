...
from commas.state import event_store_path, read_events, state_dir
...
TURN_SKIP_PREFIXES = (",", "commas ", "__commas_")
...
def session_id() -> str:
    ...
    return _session_path_component(os.environ.get("COMMAS_SESSION_ID") or "default")


def session_dir(session_id: str | None = None) -> Path:
    """Return the continuity directory for one shell session."""
    if session_id is None:
        base = os.environ.get("COMMAS_SESSION_DIR")
        if base:
            return Path(base)
    raw = session_id or os.environ.get("COMMAS_SESSION_ID") or "default"
...
def clear_current_session() -> dict[str, list[str]]:
    ...
    cleared: list[str] = []
    context = zeta_session_for_commas()
...
def latest_active_failure() -> dict[str, Any] | None:
    ...
    from commas.failure import last_failure_or_none
...
def active_failure_context(since: float | None = None) -> str:
    ...
    """
    from commas.failure import failure_context_prompt
...
def record_turn(
    ...
    """
    from commas.failure import record_failure, truncate_snippet
